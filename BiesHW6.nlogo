...
to throw-darts
  ...
  create-turtles number-of-darts [
    set shape "dart"
    set size .3
    setxy(random-float 2.0 - 1.0) (random-float 2.0 - 1.0)
  ]
  calc-pi
end

to calc-pi
  let darts-in-target count turtles with [(xcor * xcor + ycor * ycor) < 1]
  set estimated-pi 4 * darts-in-target / number-of-darts
end
@#$#@#$#@
GRAPHICS-WINDOW
211
