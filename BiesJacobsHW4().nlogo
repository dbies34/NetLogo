...
to eat-food
  ask turtle 0 [
    if (pcolor = lime) or (pcolor = yellow) or (pcolor = red) [
      set pcolor green
...
to regrow-yard
  let x random 100
  if x < 8 [
    ask n-of 1 patches [set pcolor brown] ; regrow weeds
    ask n-of 2 patches [set pcolor lime] ; regrow shoots
    ask n-of 1 patches [set pcolor yellow] ; toss fruit in
  ]
end

to make-yard
  ask patches [set pcolor green] ; make the grass
  ask n-of 6 patches [set pcolor violet] ; deer resistant
  ask n-of 5 patches [set pcolor brown] ; weeds (deer-resistant)
  ask n-of 3 patches [set pcolor red] ; apple trees
  ask n-of 6 patches [set pcolor lime] ; newly-grown shoots

end

to make-deer
  create-turtles 3 [setxy random-pxcor random-pycor]
  ask turtle 0 [ ; the buck
    set color brown
    set shape "cow"
    set size 2
  ]
  ask turtle 1 [ ; the doe
    set color yellow
    set shape "sheep"
  ]
  ask turtle 2 [ ; the fawn
    set color red
    set shape "fish"
  ]
end
