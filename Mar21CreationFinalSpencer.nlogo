...
to create-the-second-day
  ...
  output-print "God separated the water and the moisture in the air"
  output-print "Thus creating the atmosphere"
end
...
to make-water-and-land
  output-print "The third day brought the seas and the dry land"
  ask turtles [ die ]
  repeat 3 [
...
;spawn turtles, cows, and fish as the animals in the world
to create-the-fourth-day
  output-print "Let the water teem with an abundance of living creatures. "
  spawn-water-animals
end

to create-the-fifth-day
  output-print "On the Earth let cows roam free, "
  output-print "and on the beaches let turtles chill on the beaches."
  spawn-land-animals
  spawn-amphibious-animals
end

to create-the-sixth-day
  output-print "on the sixth day, God created different sizes of animals."
  ask n-of ( num-of-turtles / 2 ) tortoises [ set size 1 ]
  ask tortoises with [ size = 1.5 ] [ set size 2 ]
end

to create-the-seventh-day
  output-print "God rested on the 7th day..."
  reset-ticks
  initialize-globals
end

to create-the-eighth-day
  output-print "the eighth day brought the four seasons."
...
to what-happened-next
  ...
  ; Starting from the center, we need a semi-even distribution across the screen
  let spd 8; We will vary the speed of the stars as the move along
  while [spd > .1] [
    ask turtles with [ shape = "fire" ] [
      right 4
      forward spd * (random-float 2.2 - .5) ; Sometimes the stars travel backwards, which spreads out the stars decently
    ]
    set spd (spd / 1.2 ) ; The stars slow down every frame
    wait .05
  ]
...
to spring-move
  output-print "it is Spring. Turtles can swim anywhere."
  turtles-move-anywhere
end

to summer-move
  output-print "it is Summer. Turtles can swim anywhere."
  turtles-move-anywhere
end

to fall-move
  output-print "it is Fall. The young turtles are in shallow water, and the old are in deep water."
  move-large-turtles
  move-small-turtles
end

to winter-move
  output-print "it is Winter. The young turtles are in shallow water, and the old are in deep water."
  move-large-turtles
  move-small-turtles
end

to move-small-turtles
  ask tortoises with [ size = 1 and is-beach?] [
    let target-patch min-one-of patches in-radius 25 with [ is-beach? and ( distance myself > 1 )] [ distance myself ]
    move-to target-patch
  ]

  ask tortoises with [ size = 1 and not is-beach?] [
    let target-patch min-one-of (patches in-radius 25 with [ shallow-water? and ( distance myself > 1 ) ]) [ distance myself ]
    move-to target-patch
  ]
end

to move-large-turtles
  ask tortoises with [ size = 2 ] [
    let target-patch min-one-of (patches in-radius 25 with [ deep-water? and ( distance myself > 1 ) ]) [ distance myself ]
    move-to target-patch
  ]
end

;in the spring and summer, any turtle has the capability of swimming anywhere.
to turtles-move-anywhere
  ask tortoises [
...
to-report deep-water?
  report altitude < -200
end

to-report shallow-water?
  report altitude > -30 and altitude < 20
end
@#$#@#$#@
GRAPHICS-WINDOW
210
